...
const LoginScreen = () => {
  ...
  const submitHandler = async (e) => {
    e.preventDefault()
    try {
      const res = await login({email, password}).unwrap();// call login and get the response back
  ...
  return (
    <FormContainer>
      ...
      <Row className='py-3'>
        <Col>
          New Customer? <Link to={redirect ? `/register?redirect=${redirect}` : '/register' }>Register</Link> {/*REMOVE THIS???*/}
        </Col>
      </Row>
    </FormContainer>
  )
